...
import { Loader2, AlertCircle, Globe, Search, ExternalLink, MonitorPlay, RefreshCw, Wifi, WifiOff } from 'lucide-react';
...
const BrowserViewport = ({
  ...
  const handleClick = useCallback((e) => {
    ...
    onMouseClick?.(scaled.x, scaled.y, e.button, e.detail || 1);
  ...
  const handleKeyDown = useCallback((e) => {
    if (isInteracting) {
      // Allow default behavior for some browser shortcuts
  ...
  return (
    <div
      ref={viewportRef}
      className={cn(
        'flex-1 relative overflow-hidden bg-zinc-950',
